# 📘 GCCI — Global Climate Condition Index  
Sistema Global de Avaliação da Condição Climática  
Criado por **Wagner Cócia de Oliveira**  
Campinas — São Paulo — Brasil  
Data de criação: **16/11/2025**

---

## 🌍 O que é o GCCI?
O **GCCI** (Global Climate Condition Index) é um sistema científico global criado para medir a condição real do planeta de forma integrada, utilizando exclusivamente dados públicos de instituições internacionais como **ONU, IPCC, NASA, NOAA, FAO, ESA, IEA**, entre outras.

O índice consolida informações em **6 macrodimensões** e **25 camadas analíticas**, permitindo avaliar tendências ambientais, riscos climáticos e impactos estruturais.

---

## 🧩 Estrutura Oficial
- **6 macrodimensões globais**
- **25 camadas de medição**
- **3 tokens institucionais**
  - **GCCI-M** — Measurement Token  
  - **GCCI-P** — Protection Token  
  - **GCCI-X** — Exchange Token (uso institucional regulado)

Os tokens possuem funções estritamente **institucionais e técnicas**, não envolvendo qualquer forma de uso comercial aberto.

---

## 🏛 Governança
O **GCCI Institute** é uma entidade planejada para o futuro.  
Ainda **não existe** e será constituído conforme a evolução e adoção internacional do GCCI, com o objetivo de preservar continuidade, auditoria e integridade institucional.

---

## 🔒 Propriedade Intelectual
O GCCI é criação original de **Wagner Cócia de Oliveira**.  
Toda a metodologia interna — incluindo algoritmos, modelos matemáticos, lógica computacional, pesos, agregações e estruturas de cálculo — permanece **confidencial e proprietária**.

Este repositório contém apenas a documentação pública e institucional.

---

## 📄 Documentos Oficiais  
Localizados na pasta **/docs** deste repositório:

- **[GCCI_Official_Document_v1_4.pdf](docs/GCCI_Official_Document_v1_4.pdf)**  
- **[GCCI_Protecao_Institucional_v1_3_FULL.pdf](docs/GCCI_Protecao_Institucional_v1_3_FULL.pdf)**  

A logomarca oficial também está disponível em `docs/GCCI_Logomarca.png`.

---

## 📬 Contato
Autor: **Wagner Cócia de Oliveira**  
LinkedIn: www.linkedin.com/in/wagner-cócia-de-oliveira-32660a2a3  

---

## 📘 Licença
Este repositório é protegido pela **GCCI Public Disclosure License — v1**.  
Permite leitura e citação.  
Proíbe cópia, replicação, redistribuição, engenharia reversa ou uso comercial sem autorização.

Copyright © 2025 — Wagner Cócia de Oliveira  
Todos os direitos reservados.
